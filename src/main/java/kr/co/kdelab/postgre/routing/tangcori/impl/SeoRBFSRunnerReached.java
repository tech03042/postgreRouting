...
public class SeoRBFSRunnerReached extends SeoRBFSRunner {
    public SeoRBFSRunnerReached(int pts, int pv) {
        super(pts, pv);
    }
    ...
    int perform(int iteration, float minCost, float dist_sofar, boolean isForward) throws SQLException {
        if (isForward)
            return perform_forward(iteration, minCost, dist_sofar);
        else
            return perform_backward(iteration, minCost, dist_sofar);
    }

    private int perform_forward(int iteration, float minCost, float dist_sofar) throws SQLException {
        int affected = 0;
        int[] k = new int[2];
        String er_f_string = "";
        ...
        er_f_string = "" + "CREATE VIEW er_f_" + iteration + " (id,p2s,d2s) AS" + " "
                + "( SELECT TE.tid, TE.fid, TF.d2s+TE.cost" + " " + "FROM ta AS TF, te_" + k[0] + " AS TE"
                + " " + "WHERE TF.fwd = " + k[1] + " " + "AND TF.nid = TE.fid" + " " + "AND TF.d2s+TE.cost < "
                + (minCost - dist_sofar) + " ) ";

        // System.out.println("\t"+iteration+"\t"+k[1]+"\t"+k[0]);
        k[0]--;
        k[1]++;

        while (k[0] > 0 && k[1] <= iteration) {
            // System.out.println("forward // k[0]:"+k[0]+" k[1]:"+k[1]
            // +" forward_iteration:"+iteration);

            er_f_string += " UNION " + "(SELECT TE.tid , TE.fid, TF.d2s+TE.cost" + " "
                    + "FROM ta AS TF , te_" + k[0] + " AS TE" + " " + "WHERE TF.fwd=" + k[1] + " "
                    + "AND TF.nid=TE.fid" + " " + "AND TF.d2s+TE.cost<" + (minCost - dist_sofar) + ") ";

            k[0]--;
            k[1]++;
        } // end while

        // System.out.println(er_f_string);
        // System.in.read();
        try (Statement statement = getConnection().createStatement()) {
            statement.execute(er_f_string);
        ...
        // System.out.println("forward_affected:"+affected);
        return affected;

    } // end perform

    private int perform_backward(int iteration, float minCost, float dist_sofar) throws SQLException {
