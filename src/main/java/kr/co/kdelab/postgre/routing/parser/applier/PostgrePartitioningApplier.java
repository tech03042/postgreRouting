...
public class PostgrePartitioningApplier extends TableApplier {
    ...
    public PostgrePartitioningApplier(int pts, int pv, boolean usingFullTe) {
        this(-1);
        this.pts = pts;
        this.pv = pv;
    ...
    public void applyInTable(Connection connection) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            for (int i = 0; i < pts; i++) {
                baseSQL[i] = String.format("INSERT INTO te%d(fid, tid, cost) values", i);
                appended[i] = new StringBuilder();


                statement.execute(createTeHead + i + createTeTail);
                statement.execute("DELETE FROM TE" + i);
            }
